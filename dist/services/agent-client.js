...
export class AgentClient {
    ...
    createCustomTools() {
        return createSdkMcpServer({
            ...
            version: '1.0.0',
            tools: [
                tool('search_inbox', 'Search emails using Gmail query syntax (works with both Mock and Gmail)', {
...
- - - Exclude: invoice -draft`),
                    ...
                    try {
                        ...
                        if (hasSearchWithLogs) {
                            ...
                            const result = await this.inboxService.searchWithLogs(args.gmailQuery);
                            return {
                                content: [{
                                        type: 'text',
                                        text: JSON.stringify({
                            ...
                            const results = await this.inboxService.search({ gmailQuery: args.gmailQuery });
                ...
                tool('read_email', 'Get full details of a specific email by ID', {
                ...
                tool('read_emails', 'Read multiple emails by their IDs to get full content and details (batch operation)', {
                    ...
                    try {
                        ...
                        let emails;
                        if (hasBatchRead) {
                            emails = await this.inboxService.getByIds(args.ids);
                        }
                        else {
                ...
                tool('list_unread', 'Get a list of all unread emails', {
...
- "more friendly" → warm greeting, less corporate language`;
     ...
     */
    async querySingle(prompt, options) {
        const messages = [];
        let result = '';
        let totalCost = 0;
        let duration = 0;
