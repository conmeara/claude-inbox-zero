...
export class EmailDatabase {
  ...
  constructor(dbPath?: string) {
    // Default to ~/.claude-inbox/emails.db
    this.dbPath = dbPath || path.join(os.homedir(), '.claude-inbox', 'emails.db');
  ...
  insertEmail(
    ...
    const insertEmail = this.db.prepare(`
      INSERT OR REPLACE INTO emails (
        ...
        attachment_count, folder, labels, raw_headers, requires_response
      ) VALUES (
        @message_id, @thread_id, @in_reply_to, @email_references,
        @date_sent, @date_received, @subject, @from_address, @from_name,
        @reply_to, @body_text, @body_html, @snippet,
    ...
    const insertTransaction = this.db.transaction(() => {
      const result = insertEmail.run({
        ...
        requires_response: email.requires_response ? 1 : 0,
      });

      const emailId = result.lastInsertRowid as number;

      // Delete old recipients for this email (in case of update)
      this.db.prepare('DELETE FROM recipients WHERE email_id = ?').run(emailId);
      ...
      this.db.prepare('DELETE FROM attachments WHERE email_id = ?').run(emailId);
  ...
  getByMessageId(messageId: string): any | null {
    const stmt = this.db.prepare(`
      SELECT e.*,
        GROUP_CONCAT(CASE WHEN r.type = 'to' THEN r.address END) as to_addresses,
        GROUP_CONCAT(CASE WHEN r.type = 'to' THEN r.name END) as to_names
      FROM emails e
  ...
  markAsRead(messageIds: string[]): void {
    const stmt = this.db.prepare('UPDATE emails SET is_read = 1 WHERE message_id = ?');
  ...
  // Get unread emails
  getUnreadEmails(limit: number = 50): any[] {
    return this.advancedSearch({ isUnread: true, limit });
  }
  ...
  searchByDomain(domain: string, limit: number = 30): any[] {
    ...
    return query.all(domain.toLowerCase(), limit);
  }
  ...
  searchByContact(email: string, limit: number = 30): any[] {
    ...
    return query.all(email.toLowerCase(), email.toLowerCase(), limit);
  }
  ...
  searchByThread(threadId: string): any[] {
    ...
    return query.all(threadId);
  }

  // Full-text search in subject and body
  searchByKeyword(keyword: string, limit: number = 30): any[] {
    const query = this.db.prepare(`
      ...
      LIMIT ?
    `);
    return query.all(keyword, limit);
  }
  ...
  // Clear all emails (useful for reset)
  clearAll(): void {
